...
description: How the Cosmograph UI version is set and what to update when shipping features.
---
...
hand-written flight log in `artifacts/cosmograph/src/data/changelog.ts`. The
version only changes when a new entry is prepended to that array.
...
`artifacts/cosmograph/package.json` `version` to match. They are two separate
